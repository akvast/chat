...
namespace server {
    ...
    void CServerHandler::on_message(std::shared_ptr<CMessage> message) {
    ...
    void CServerHandler::send_auth_secceed(std::string name) {
    ...
    void CServerHandler::send_public_key() const {
        auto socket = get_socket();
        if (!socket)
            return;
    ...
    void CServerHandler::handle_encrypt_key(std::vector<uint8_t> data) {
        auto socket = get_socket();
        if (!socket)
            return;
    ...
    void CServerHandler::send_decrypt_key() {
        auto socket = get_socket();
        if (!socket)
            return;
    ...
    void CServerHandler::handle_auth(std::shared_ptr<CMessage> message) {
    ...
    void CServerHandler::handle_register(std::shared_ptr<CMessage> message) {
